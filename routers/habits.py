...
async def api_add_habit(data: HabitRequest, user=Depends(get_current_user)):
    if data.description:
        ...
        await socketio_server.emit("data_updated", {"message": "Habit added"}, to=str(user.id))
...
async def api_complete_habit(habit_id: int, user=Depends(get_current_user)):
    ...
    if success:
        await socketio_server.emit("data_updated", {"message": "Habit completed"}, to=str(user.id))
        return {"message": f"Habit {habit_id} completed for today."}
    raise HTTPException(status_code=400, detail="Habit not found or already completed today.")
...
async def api_delete_habit(habit_id: int, user=Depends(get_current_user)):
    ...
    if success:
        await socketio_server.emit("data_updated", {"message": "Habit deleted"}, to=str(user.id))
