<template>
  ...
  <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 transition-colors">
    ...
    <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-100 border-b border-gray-100 dark:border-gray-700 pb-3 mb-6 transition-colors">Privacy</h3>
    ...
    <div class="flex items-center justify-between">
      ...
      <Switch
        ...
        :class="user.is_public ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'"
        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
      >
        <span class="sr-only">Make profile public</span>
        <span
          :class="user.is_public ? 'translate-x-6' : 'translate-x-1'"
          class="inline-block h-4 w-4 transform rounded-full bg-white transition"
        />
      </Switch>
    </div>
...
const savePrivacy = async (newValue: boolean) => {
  ...
  if (res.ok) {
    showToast("Privacy updated successfully.")
  } else {
